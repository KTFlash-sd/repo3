  ...
  <tr>
    ...
    <%
     ...
     for(Menu menu:menulist){ 
    	if(menu.getMstate()==1&&menu.getMfid()==0){
    %>
     <dl class='bitem'>
        ...
        <dd style='display:block' class='sitem' id='items<%=i%>_1'>
          <ul class='sitemu'>
            <% for(Menu menu2:menulist){
                if(menu2.getMstate()==1&&menu2.getMfid()==menu.getMid()){
               %>
            <li><a href='${pageContext.request.contextPath}<%=menu2.getMurl() %>' target='main'><%=menu2.getMname() %></a> </li>
            <% 
                }
            }%>
          </ul>
