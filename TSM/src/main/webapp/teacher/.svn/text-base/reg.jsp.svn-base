...
// 				return;
	...
	function ajaxs(val){
		...
		$.ajax({
			...
			data:"method=queryTeacher&thispage="+val,
			success:function(data){
				var pagebean=data.pagebean;
				var rows=data.rows;
				$.each(rows,function(index,teacher)){
					$("#table").append("<tr><td>"+teacher.Tname+"</td><td>"+teacher.Tsex+"</td><td>"+teacher.Tem+"</td><td>"+teacher.Tbirth+"</td><td>"+teacher.Tps+"</td><td>"+teacher.Tnative+"</td><td>"+teacher.Ttel
					+"</td><td>"+teacher.Tmajor+"</td><td>"+teacher.Tyear+"</td><td>"+teacher.Tiob+"</td></tr>");
	...
	<form id="submitForm" name="submitForm" action="" method="post">
     ...
     <script>
      ...
      function ajaxmethod(value){
    	$("#t tr:not(.one)").remove(); //首先移除表格中除第一行之外的其他行 
    	  $.ajax({
    		  ...
    		  data:"method=queryTeacher&thispage="+value,//给method和thispage赋值
    		  success:function(data){
    			 var pagebean=data.pagebean;
    			 var rows=data.rows;
    			 $.each(rows,function(index,teacher){
    				 $("#t").append("<tr><td>"+teacher.tname+"</td><td>"+teacher.tsex+"</td><td>"+teacher.tem+"</td><td>"+teacher.tbirth+"</td><td>"+teacher.tps+"</td><td>"+teacher.tnative+"</td><td>"+teacher.ttel
      ...
      function del(tname){
     	 $.ajax({
     		 url:"${pageContext.request.contextPath}/teacher",
     		 data:"method=deleteTeacher&Tname="+tname,
     		 success:function(data){
     			 if(data>0){
     				 ajaxmethod($("#p").html());
     			 }
